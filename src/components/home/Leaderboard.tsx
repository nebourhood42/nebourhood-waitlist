...
const LeaderBoardContent = ({ name, image, _key_, color, icon, showNumbering = true, showPercentile = false, count, triggered }: { name: string, image?: StaticImageData | string, color: string, _key_: string, icon?: string | null, showNumbering?: boolean, showPercentile?: boolean, count?: number, triggered: boolean }) => (
   <div key={name} className='flex flex-col items-center'>
      <div className='flex flex-col items-center mb-8.5 md:mb-20.75'>
         <div className={`overflow-hidden w-[77px] h-[77px] mb-1.5 rounded-full ${icon ? 'w-20 h-20 flex items-center justify-center shadow-[inset_0px_4px_11px_0px_rgba(110,110,110,0.25)]' : 'border-[3px]'}`} style={{ borderColor: color }}>
            {icon && <Icon icon={icon} className={icon === 'streamline-logos:x-twitter-logo-block' ? 'invert' : ''} width={24} height={24} />}
            {image && (
               typeof image === 'string' ? (
                  <img src={image} alt='waitlist leaderboard image' className='object-cover w-full h-full' />
               ) : (
                  <Image src={image} alt='waitlist leaderboard image' className='object-cover' />
               )
            )}
         </div>
         <h2 className='text-[#093645] text-lg md:text-xl font-base text-center leading-tight'>
            {name}
            {count !== undefined && (
               <span className="block text-xs font-semibold opacity-70 mt-1">{count} {count === 1 ? 'referral' : 'referrals'}</span>
            )}
         </h2>
      </div>
      ...
      <div className='w-[58.4px] md:w-27.5 h-[220.3px] md:h-[414.8px] flex justify-center flex-col rounded-full mb-2 md:mb-4 overflow-hidden relative' style={{
         ...
         <div
            ...
            style={{
               background: color,
               top: triggered
                  ? 100 - ((5 - Number(_key_.split('_')[1])) * 25) + '%'
                  : '100%',
               transition: 'top 1s cubic-bezier(0.34, 1.26, 0.64, 1)',
               transitionDelay: `${Number(_key_.split('_')[1]) * 120}ms`,
            }}
         />
...
const Leaderboard = () => {
      ...
      const observer = new IntersectionObserver(
         ([entry]) => {
            if (entry.isIntersecting) {
               // Small delay so the bars are visible before they start filling
   ...
   useEffect(() => {
      // Re-trigger bar animations when toggling between leaderboard tabs for a premium feel
      if (triggered) {
         setTriggered(false);
   ...
   const getWaitlistLeaderboard = () => {
      const baseUsers: { name: string; count: number; image: StaticImageData | string; color: string }[] = [
         { name: 'Daniel', count: 15, image: img, color: '#8D268D' },
         { name: 'Chiboy', count: 10, image: img, color: '#FF637E' },
         { name: 'Kamsy', count: 5, image: img, color: '#F4A40E' }
      ];

      if (user) {
         const userIndex = baseUsers.findIndex(u => u.name.toLowerCase() === user.fullName.toLowerCase());
         if (userIndex !== -1) {
            baseUsers[userIndex].count = user.referral_count;
         } else {
            baseUsers.push({
               name: user.fullName || 'You',
               count: user.referral_count,
               image: user.profileImage || img,
               color: '#000000'
            });
         }
      }

      baseUsers.sort((a, b) => b.count - a.count);

      // Return in order n_3, n_1, n_2 to preserve podium visual structure (3rd, 1st, 2nd)
      return {
         "n_3": {
            name: baseUsers[2].name,
            image: baseUsers[2].image,
            color: '#F4A40E',
            count: baseUsers[2].count
         },
         "n_1": {
            name: baseUsers[0].name,
            image: baseUsers[0].image,
            color: '#8D268D',
            count: baseUsers[0].count
         },
         "n_2": {
            name: baseUsers[1].name,
            image: baseUsers[1].image,
            color: '#FF637E',
            count: baseUsers[1].count
         }
      };
   };

   const getInvitesLeaderboard = () => {
      const baseUsers: { name: string; count: number; image: StaticImageData | string; color: string }[] = [
         { name: 'Daniel', count: 15, image: img, color: '#4584E1' },
         { name: 'Chiboy', count: 10, image: img, color: '#268D69' },
         { name: 'Kamsy', count: 5, image: img, color: '#8A38F5' }
      ];
   ...
   const waitlistLeaderboard = getWaitlistLeaderboard();
   const invitesLeaderboard = getInvitesLeaderboard();
   ...
   return (
      <aside ref={sectionRef} className="py-25 lg:pt-35 pb-22 lg:pb-35 bg-background">
            ...
            <div className="flex justify-center gap-11.5 md:gap-21.5 bg-foreground rounded-[40px] py-11.5 md:py-15 mb-4 md:mb-10">
               {
                  tab == 'waitlist' ? (
                     Object.entries(waitlistLeaderboard).map(([key, value]) => (
                        <LeaderBoardContent name={value.name} image={value.image} key={tab+key} color={value.color} _key_={key} count={value.count} triggered={triggered} />
                     ))
                  ) : tab == 'invites' ? (
                     Object.entries(invitesLeaderboard).map(([key, value]) => (
                        <LeaderBoardContent name={value.name} image={value.image} key={tab+key} color={value.color} _key_={key} showNumbering={false} showPercentile count={value.count} triggered={triggered} />
                     ))
                  ) : (
                     Object.entries(socialsLeaderboard).map(([key, value]) => (
                        <LeaderBoardContent name={value.name} icon={value.icon} key={tab+key} color={value.color} _key_={key} triggered={triggered} />
                     ))
                  )
               }
            </div>
